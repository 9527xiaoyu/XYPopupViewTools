...
+ (void)popupTopWithTargetView:(UIView *)targetView maskClick:(BOOL)maskClick  maskAlpha:(CGFloat)maskAlpha
{
    ...
    [self popupTopWithFrom:window targetView:targetView maskAlpha:maskAlpha showAnima:XYAnimationStyleTop hideAnima:XYAnimationStyleTop maskClick:maskClick];
}
...
{
    [self popupViewWithFrom:fromView targetView:targetView maskStyle:XYMaskSTypeAlpha  maskAlpha:maskAlpha showStyle:XYTargetShowStyleTop showAnima:showAnimaStyle hideAnima:hideAnimaStyle maskCanClick:maskClick completed:^{
...
+ (void)popupCenterWithTargetView:(UIView *)targetView maskClick:(BOOL)maskClick  maskAlpha:(CGFloat)maskAlpha
{
    UIWindow *window = [UIApplication sharedApplication].keyWindow;
    window.windowLevel = UIWindowLevelAlert;
    [self popupCenterWithFrom:window targetView:targetView maskAlpha:maskAlpha showAnima:XYAnimationStyleCenter hideAnima:XYAnimationStyleCenter maskClick:maskClick];
}
...
{
    [self popupViewWithFrom:fromView targetView:targetView maskStyle:XYMaskSTypeAlpha maskAlpha:maskAlpha showStyle:XYTargetShowStyleCenter showAnima:showAnimaStyle hideAnima:hideAnimaStyle maskCanClick:maskClick completed:^{
...
+ (void)popupBottomWithTargetView:(UIView *)targetView maskClick:(BOOL)maskClick maskAlpha:(CGFloat)maskAlpha
{
    UIWindow *window = [UIApplication sharedApplication].keyWindow;
    window.windowLevel = UIWindowLevelAlert;
    [self popupBottomWithFrom:window targetView:targetView maskAlpha:maskAlpha showAnima:XYAnimationStyleBottom hideAnima:XYAnimationStyleBottom maskClick:maskClick];
}
...
{
    [self popupViewWithFrom:fromView targetView:targetView maskStyle:XYMaskSTypeAlpha maskAlpha:maskAlpha showStyle:XYTargetShowStyleBottom showAnima:showAnimaStyle hideAnima:hideAnimaStyle maskCanClick:maskClick completed:^{
...
+ (void)centerPopupAnimation:(BOOL)show
{
    if (show) {//显示
        _targetView.transform = CGAffineTransformScale(CGAffineTransformIdentity,
                                                       CGFLOAT_MIN, CGFLOAT_MIN);
        [UIView animateWithDuration:kAnimDuration animations:^{
            _targetView.transform =
            CGAffineTransformScale(CGAffineTransformIdentity, 1.5, 1.5);//扩大1.5倍
        } completion:^(BOOL finished) {
            [UIView animateWithDuration:kAnimDuration animations:^{
                _targetView.transform = CGAffineTransformIdentity;//恢复原样
            } completion:^(BOOL finished) {
                _completed?_completed():nil;
            }];
        }];
    }else{//隐藏
        _targetView.transform = CGAffineTransformIdentity;//原样
        [UIView animateWithDuration:0.2 animations:^{
            _targetView.transform =
            CGAffineTransformScale(CGAffineTransformIdentity, 0.1, 0.1);//缩小0.1倍
        } completion:^(BOOL finished) {
            [UIView animateWithDuration:0.2 animations:^{
                _targetView.transform = CGAffineTransformScale(CGAffineTransformIdentity,
                                                               CGFLOAT_MIN, CGFLOAT_MIN);//无限小
            } completion:^(BOOL finished) {
                _targetView.transform = CGAffineTransformIdentity;//原样
                [self removed];
            }];
